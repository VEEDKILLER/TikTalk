"""
serve.py — Modal deployment for TikTalk ASR inference using faster-whisper.

Deployment:
    modal deploy asr_deploy/serve.py

Upload model to Modal Volume:
    modal volume put tiktalk-asr-model ct2_model/ /

Test locally (dev mode):
    modal serve asr_deploy/serve.py
"""
...
class WhisperASR:
    ...
    def load_model(self):
        ...
        self.model = WhisperModel(MODEL_MOUNT_PATH, device="cuda", compute_type=COMPUTE_TYPE)
...
async def transcribe_audio(audio: UploadFile = File(...)):
    ...
    if file_ext not in ALLOWED_EXTENSIONS:
        return JSONResponse(
            status_code=400,
            content={"error": f"Unsupported format: '{file_ext}'."},
        )
